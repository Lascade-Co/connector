...
def get_client(
    ...
    # use service account to authenticate if not using OAuth2.0
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            return GoogleAdsClient.load_from_dict(config_dict=config)
...
def customers(
        client: GoogleAdsClient, customer_id: str = dlt.secrets.value
) -> Iterator[TDataItem]:
    """
    ...
    query = "SELECT customer.id, customer.descriptive_name FROM customer"  # noqa
...
def campaigns(
    ...
    # Issues a search request using streaming.
    ga_service = client.get_service("GoogleAdsService")
    query = "SELECT campaign.id, campaign.labels FROM campaign"  # noqa
    stream = ga_service.search_stream(customer_id=customer_id, query=query)
    for batch in stream:
        for row in batch.results:
...
def change_events(
    ...
    # Issues a search request using streaming.
    ga_service = client.get_service("GoogleAdsService")
    query = "SELECT change_event.change_date_time FROM change_event WHERE change_event.change_date_time during LAST_14_DAYS LIMIT 1000"  # noqa
    stream = ga_service.search_stream(customer_id=customer_id, query=query)
    for batch in stream:
        for row in batch.results:
...
def customer_clients(
    ...
    # Issues a search request using streaming.
    ga_service = client.get_service("GoogleAdsService")
    query = "SELECT customer_client.status FROM customer_client"  # noqa
    stream = ga_service.search_stream(customer_id=customer_id, query=query)
    for batch in stream:
        for row in batch.results:
